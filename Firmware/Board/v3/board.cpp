...
// 从技术上讲，这应该在task_timer.cpp中，但我们不要创建一个一行文件  
// 
...
// 该数组放置在 ram (0x20000000) 的最开始处，并将在 manufacturing 期间
// 在实际写入 OTP 前用于测试现有的 OTP 结构. 
// 如果我们在制造脚本中引入了意外损坏，这可以避免批量破坏 STM32。 
...
OnboardThermistorCurrentLimiter fet_thermistors[AXIS_COUNT] = {
    {
        15, // adc_channel 轴 0 的 ADC 通道
        &fet_thermistor_poly_coeffs[0], // coefficients 多项式系数
        fet_thermistor_num_coeffs // num_coeffs 多项式系数数量
    }, {
#if HW_VERSION_MAJOR == 3 && HW_VERSION_MINOR >= 3
        4, // adc_channel 轴 1 的 ADC 通道
#else
        1, // adc_channel
#endif
        &fet_thermistor_poly_coeffs[0], // coefficients 多项式系数
...
std::array<Axis, AXIS_COUNT> axes{{
    {
        0, // axis_num
        1, // step_gpio_pin step 引脚
        2, // dir_gpio_pin dir 引脚
        (osPriority)(osPriorityHigh + (osPriority)1), // thread_priority 优先级
        encoders[0], // encoder 编码器
        sensorless_estimators[0], // sensorless_estimator 无感估计器
        controllers[0], // controller 控制器
        motors[0], // motor 电机
        trap[0], // trap 梯形轨迹（规划器?）
        endstops[0], endstops[1], // min_endstop, max_endstop
        mechanical_brakes[0], // mechanical brake 机械刹车
    },
    {
        1, // axis_num
...
Stm32Gpio gpios[] = {
    {nullptr, 0}, // dummy GPIO0 so that PCB labels and software numbers match 伪GPIO0使PCB标签和软件编号匹配  
...
#else
PwmInput pwm0_input{&htim5, {1, 2, 3, 4}}; // pwm输入及对应的软件引脚
#endif
...
void system_init() {
    ...
    // 如果 OTP 是原始的，改用 RAM 中的 fake-otp  
...
bool board_init() {
    ...
    if (odrv.config_.enable_can_a) {
        ...
        // CAN 初始化将（并且必须）在 GPIO 模式初始化之前初始化其自己的 GPIO。 
        // 因此我们确保后面的 GPIO 模式初始化不会覆盖 CAN 模式。
    ...
    // 重置两个 DRV 芯片。 使能引脚还控制 SPI 接口，而不仅仅是驱动级。
    drv_enable_gpio.write(false);
    delay_us(40); // mimumum pull-down time for full reset: 20us 完全复位的最小下拉时间: 20us
...
void start_timers() {
    CRITICAL_SECTION() {
        ...
        * 
        * 同步TIM1, TIM8和TIM13，这样:  
        * 1。 TIM1的三角波形比TIM8的三角波形超前90°相移。  
        * 2。 每次TIM13重载都与一个TIM1较低更新事件相一致。
        * 
        */
...
void TIM8_UP_TIM13_IRQHandler(void) {
    ...
    // update event.
    // 在定时器更新事件后 21-23 个时钟周期进入此函数。
    __HAL_TIM_CLEAR_IT(&htim8, TIM_IT_UPDATE);

    // If the corresponding timer is counting up, we just sampled in SVM vector 0, i.e. real current
    // 如果相应的定时器正在计数，我们只是在 SVM 向量 0 中采样，即实际电流
    // If we are counting down, we just sampled in SVM vector 7, with zero current
    // 如果我们倒计时，我们只是在 SVM 向量 7 中采样，电流为零
    ...
    if (!counting_down) {
        ...
        // 暂时将所有 PWM 输出重置为 50% 占空比。 
        // 如果控制循环处理程序及时完成，那么这些值将在它们生效之前被覆盖。
        TIM1->CCR1 =
        TIM1->CCR2 =
        TIM1->CCR3 =
        TIM8->CCR1 =
...
void ControlLoop_IRQHandler(void) {
    ...
    // 如果电机 FET 没有切换，则我们无法测量电流
    // 因为为此我们需要低侧 FET 来导通。
    // 所以现在我们猜测电流为 0（这在去使能后不久和电机在空闲状态下快速旋转时是不正确的）。
    // 传递无效的电流读数会在启动 FOC 时产生问题。
    ...
    // 如果我们做的一切正确，TIM8 更新处理程序应该在这个函数的开始和现在之间被调用一次。
